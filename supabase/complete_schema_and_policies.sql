...
-- Table: assistant_logs (for AI assistant interactions)
...
-- Assistant Logs
CREATE POLICY "Admin can view own assistant logs" ON public.assistant_logs
FOR SELECT 
USING (
  user_id = auth.uid() 
  AND EXISTS (
...
CREATE POLICY "Admin can insert own assistant logs" ON public.assistant_logs
FOR INSERT 
WITH CHECK (
  user_id = auth.uid() 
  AND EXISTS (
...
CREATE POLICY "Super admin can view all assistant logs" ON public.assistant_logs
FOR SELECT 
USING (
  EXISTS (
