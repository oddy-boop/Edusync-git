...
import { Users, Edit, Trash2, ChevronDown, UserCog, Search, Loader2, AlertCircle, RefreshCw, FileDown } from "lucide-react";
...
export default function AdminUsersPage() {
  ...
  const [viewMode, setViewMode] = useState<string>("Full Year"); // "Full Year", "Term 1", etc.
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    ...
    let tempStudents = [...allStudents].map(student => {
      ...
      if (viewMode === "Full Year") {
        feesDue = feeStructureForCurrentYear
        ...
        amountPaid = studentPaymentsThisYear.reduce((sum, p) => sum + p.amount_paid, 0);

      } else { // Term-specific view
        feesDue = feeStructureForCurrentYear
          .filter(item => item.grade_level === student.grade_level && item.term === viewMode)
          .reduce((sum, item) => sum + item.amount, 0);

        // This is a simplified calculation: sum of all payments tagged for this term.
        // A full ledger system would be more complex.
        const studentPaymentsThisTerm = allPaymentsFromSupabase.filter(p => 
            p.student_id_display === student.student_id_display && 
            p.term_paid_for.includes(viewMode) &&
            academicYearStartDate && p.payment_date >= academicYearStartDate &&
            academicYearEndDate && p.payment_date <= academicYearEndDate
        );
        amountPaid = studentPaymentsThisTerm.reduce((sum, p) => sum + p.amount_paid, 0);
      }
  ...
  const handleResetOverrides = async () => {
    ...
    try {
        const { error } = await supabase
            ...
            .neq('total_paid_override', null); // Only update rows that actually have an override

        if (error) throw error;

        toast({ title: "Success", description: "All student payment overrides have been reset. Balances are now calculated from actual payments." });
        await loadAllDataFromSupabase(); // Refresh data to reflect changes
    } catch (e: any) {
        toast({ title: "Error", description: `Could not reset overrides: ${e.message}`, variant: "destructive" });
    } finally {
  ...
  useEffect(() => {
    const generatePdf = async () => {
        if (studentForStatement && pdfRef.current && schoolBranding) {
            setIsDownloading(true);
            const element = pdfRef.current;
            const opt = {
                margin: 0.5,
                filename: `Fee_Statement_${studentForStatement.full_name.replace(/\s+/g, '_')}_${currentSystemAcademicYear}.pdf`,
    ...
    generatePdf();
  }, [studentForStatement, schoolBranding, currentSystemAcademicYear]);
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
           ...
           <p className="col-span-4 text-xs text-muted-foreground px-1 text-center sm:text-left sm:pl-[calc(25%+0.75rem)]">
            Note: This overrides the calculated total paid for the entire year. To reset, leave blank and save.
          </p>
        </div>
        <DialogFooter>
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader><CardTitle>Registered Students (from Supabase)</CardTitle><CardDescription>View, edit, or delete student records. Use the filter to view fee status by term or for the full year.</CardDescription></CardHeader>
        <CardContent>
          <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto"><Label htmlFor="viewMode">View:</Label><Select value={viewMode} onValueChange={setViewMode}><SelectTrigger id="viewMode"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Full Year">Full Year</SelectItem>{TERMS_ORDER.map(term => <SelectItem key={term} value={term}>{term}</SelectItem>)}</SelectContent></Select></div>
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="destructive" className="w-full sm:w-auto"><RefreshCw className="mr-2"/> Reset All Overrides</Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                  <AlertDialogHeader><AlertDialogTitle>Confirm Reset</AlertDialogTitle><AlertDialogDescription>Are you sure you want to reset ALL student payment overrides? This will remove any manually entered "Total Paid" amounts and recalculate all yearly balances from actual payment records. This action cannot be undone.</AlertDialogDescription></AlertDialogHeader>
                  <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction onClick={handleResetOverrides} disabled={isResettingOverrides}>
                        {isResettingOverrides ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : null}
                        Yes, Reset Overrides
                      </AlertDialogAction>
                  </AlertDialogFooter>
              </AlertDialogContent>
          ...
          {isLoadingData ? <div className="py-10 flex justify-center"><Loader2 className="h-8 w-8 animate-spin"/> Loading student data...</div> : (
            <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Display ID</TableHead><TableHead>Name</TableHead><TableHead>Grade</TableHead><TableHead>Fees Due ({viewMode})</TableHead><TableHead>Paid ({viewMode})</TableHead><TableHead>Balance ({viewMode})</TableHead><TableHead>Guardian Contact</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
              <TableBody>{filteredAndSortedStudents.length === 0 ? <TableRow key="no-students-row"><TableCell colSpan={8} className="text-center h-24">No students found.</TableCell></TableRow> : filteredAndSortedStudents.map((student) => {
                    const displayTotalPaid = (viewMode === "Full Year" && student.total_paid_override !== null && student.total_paid_override !== undefined) 
                    ...
                    return (<TableRow key={student.id}><TableCell>{student.student_id_display}</TableCell><TableCell>{student.full_name}</TableCell><TableCell>{student.grade_level}</TableCell><TableCell>{feesDue.toFixed(2)}</TableCell><TableCell>{displayTotalPaid.toFixed(2)}{viewMode === 'Full Year' && student.total_paid_override !== null && student.total_paid_override !== undefined && <span className="text-xs text-blue-500 ml-1">(Overridden)</span>}</TableCell><TableCell className={balance > 0 ? 'text-destructive' : 'text-green-600'}>{balance.toFixed(2)}</TableCell><TableCell>{student.guardian_contact}</TableCell><TableCell className="space-x-1">
                        ...
                        <Button variant="outline" size="icon" onClick={() => handleDownloadStatement(student)} disabled={isDownloading && studentForStatement?.id === student.id} title="Download Fee Statement">
                            {isDownloading && studentForStatement?.id === student.id ? <Loader2 className="h-4 w-4 animate-spin"/> : <FileDown className="h-4 w-4"/>}
                        </Button>
